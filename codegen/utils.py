...
def extract_latest_versions(definitions: Dict[str, Any]) -> Dict[str, Dict[str, str]]:
    ...
    for key in definitions.keys():
        ...
        if match:
            ...
            if base_name not in latest_versions or version_suffix > latest_versions[base_name].get("version_suffix", ""):
...
def method_type_from_method_name(method_name: str) -> str:
    ...
    if (method_name_lower.startswith("get") or method_name_lower.startswith("list") or 
        method_name_lower.startswith("test") or method_name_lower.startswith("tget") or 
        method_name_lower.startswith("tlist") or method_name_lower.startswith("ttest")):
        return "query"
    return "command"
